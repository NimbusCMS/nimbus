...
- [x] **OAuth SSO — Phase 1** ([ADR 0012](docs/adr/0012-oauth-sso.md)) — "Sign in with Google/GitHub" as an opt-in, **off-by-default** core subsystem. Authorization-Code + PKCE (S256) + single-use session-bound `state`; identity from the userinfo endpoint over TLS-verified curl (no JWT library); `nb_oauth_identities` keyed on the immutable provider subject, `UNIQUE`, no tokens stored. Phase 1 = **sign-in for a linked identity** + **explicit link from Settings** only; an unknown identity is rejected (no auto-provision, email never a key); password login always available. `OAuthProvider` interface + Google/GitHub adapters; 16 controls-as-tests via a fake provider on the real kernel. **Deferred (each own review):** P2 invite-accept via provider · P3 opt-in verified-email auto-link · P4 opt-in allow-list signup
- [x] **Trusted-proxy URL handling** — resolved deliberately: generated absolute URLs stay **env-authoritative** (`APP_URL`), never derived from a spoofable forwarded Host, so reset/invite/OAuth links can't be poisoned (guarded by a tripwire test). The trusted-proxy signal instead powers a **misconfiguration diagnostic** on the admin Plugins page (`APP_URL` still localhost, or `http://` while the request is HTTPS-via-proxy). `Request::viaTrustedProxy()` added; no forwarded-host accessor by design
...
- [x] **Settings store** — DB-backed, admin-editable site settings (`site.home`, `site.description`) over a typed registry (allow-list; only known keys writable), `config/*.php` the default the DB overrides (BC, no seed migration); `settings:write` capability + admin form + MCP `get_settings`/`set_settings`; deploy/env config kept in files. Security-green (both skills; `SettingsSiteTest`/`SiteSettingsTest`/`McpSettingsToolsTest`). Site title (`APP_NAME`) deferred as a fast-follow
...
- [x] **Slice 3b (fast-follow)** — gate the admin media page on `media:*` (`MediaController`: index→`media:read`, store/destroy→`media:write` per-action; nav + dashboard media card hidden without `media:read`). Behavior-preserving seed refresh: `RoleSeeder::CONTENT_MEDIA_CAPS` folds `media:read`+`media:write` into editor/author (fresh installs) and **migration 012** — the repo's first data migration — additively backfills already-seeded installs (idempotent, system-roles-only, never removes). Brings the admin surface to parity with the MCP media tools.
...
- [x] **Slice 4b-UI** — the admin token-form **role dropdown** (PR #120): mint a role-bound token from the web form (grantable-roles-only dropdown; server-side subset-only over the full role cap set + the scopes check kept; role shown in the list). Security-green (7 `TokenAdminTest` cases). Built in the finished design language. **Completes ADR 0011 roles-for-tokens across all surfaces (admin + CLI + MCP).**
...
- [x] **Increment 2 — the signatures** (PR #106): Charm Line (gold h1 underline), sidebar horizon glow + one twinkling star, constellation empty states, the token Reveal (gold-washed scroll + one-time shimmer), the "summoned in N ms" Whisper (`NB_START`). Both animations compositor-only + reduced-motion-dead. Pure CSS + 3 template tweaks; no security surface. Verified live.
- [x] **Increment 3 — the theme system + Nocturne** (PR #116): `AdminTheme` allow-list (sanitized at write + render), `data-theme` on `<html>`, the Nocturne dark-mode `[data-theme]` token block (color-scheme:dark + inversions), a new `SettingsController` with the accessible swatch picker → `POST /admin/settings/theme` (CSRF + allow-list + self-only `nb_users.theme` write, no capability gate). **Security-green** (both skills; 8 `SettingsThemeTest` cases). Byte ceiling **raised 24→28 KB** (stated design figure; inlined CSS ~5.5 KB gzipped). Verified live incl. the Nocturne mobile drawer. **Slice 4b-UI (token-form role dropdown) is a SEPARATE follow-up** (its own security surface — the roadmap's "rides Increment 3" was loose sequencing).
- [x] **Increment 4 — Daybreak + Grimoire** (PR #118): two more `[data-theme]` token blocks (dawn-blue + sun-gold; bottle-green + parchment + brass) + swatch chips + the 8-line instant-preview JS. `theme.css` 29,293 B; ceiling **settled at 30 KB** (final for the complete four-theme set). **Theme track ✅ COMPLETE** (Inc 1 tokens → Inc 2 signatures → Inc 3 theme system + Nocturne → Inc 4). Four themes: Nimbus / Nocturne / Daybreak / Grimoire.
