...
| `Nimbus\Plugin\ViewDataRegistrar` | registering **view-data contributors** for themed content pages (`PluginContext::viewData()`, ADR 0027) — the body-data sibling of the head seam. A contributor returns an array merged under the plugin's own namespace into the theme's `contrib` view key (never a core template var). It is **data the theme escapes on render** (not HTML), and **must be visitor-independent** — content pages are page-cached, so per-visitor data (cart, session, current user) would leak across visitors; the `PageContext` it receives carries none. A throwing contributor is logged and skipped (never a 500), and rolls back with a failed plugin |
...
| `Nimbus\Plugin\ServiceRegistrar` | **typed service ports** between plugins (`PluginContext::services()`, ADR 0019). `provide($contract, $impl)` publishes an implementation of an **interface** contract; `get($contract)` obtains another plugin's by that type (generic `class-string<T> → ?T`, so the result is typed). One provider per contract; an unprovided contract returns `null` (fail-safe — a consumer degrades softly). For synchronous plugin-to-plugin calls (Commerce → Inventory) that events can't do; **not** a generic service locator (interface-only, plugin-only) |
| `Nimbus\Site\HeadContributor` | the head-contribution contract (ADR 0004) |
| `Nimbus\Site\ViewDataContributor` | the view-data-contribution contract (ADR 0027) — `data(PageContext): array`, visitor-independent, escaped-on-render |
| `Nimbus\Site\PageContext` | the page data a head **or** view-data contributor receives |
