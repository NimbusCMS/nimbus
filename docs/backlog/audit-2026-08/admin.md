...
### ADMIN-7 · A failed or expired invite cannot be re-sent — the UI's own advice ("re-invite") is impossible ✅ RESOLVED
- **Resolved:** Slice I. New `POST /admin/users/{id}/invite` (`UsersController::resendInvite`) — CSRF + `users:write` + `{id}` int-cast + `find()` (benign "No such user." redirect) + sends to the **stored** email, reusing `InvitationService::sendInvite` (which supersedes only prior invite tokens). **Subset-only guard** mirrors `update()` (reject before issuing token/mail if the target holds an ungrantable role) and it is **gated to genuinely-pending users**: the signal is an unused `purpose='invite'` row in `nb_password_resets` (`PasswordResetRepository::hasPendingInvite` / `pendingInviteUserIds`) — exists from issue (so a delivery failure still counts), survives expiry (an expired invite is exactly what's resent), cleared on accept. So resend can never arm a password-set link for an active account (both reviews' Q1/Q2). A "Resend invite" row action shows only for pending users; `invited-nomail` copy corrected + `invite-sent`/`invite-nomail` flashes added. UI-only — the deferred `invite_user` MCP tool (ROADMAP) stays deferred. Tests: `UserInvitationTest` (resend pending, expired-still-resendable, active-refused, CSRF, users:write, subset guard, nonexistent id, list shows only for pending). Throttle recorded as **FU-8** (Low — pending-gate already bounds abuse).
- **Priority:** P2
- **Type:** product-gap
