...
### PLUG-5 · The CSP nonce is not part of the plugin surface — script-bearing head contributions and admin pages cannot work ✅ RESOLVED
- **Resolved:** Slice H (branch `slice-h-csp-nonce-cache`). `PageContext` gained a required `public readonly string $cspNonce` (5th param, data-only — consistent with its contract; SECURITY docblock marks it the one trusted emit-into-attribute value), passed as `Csp::nonce()` from both `SiteController` construction sites, so a head contributor can nonce its `<script>`. Admin-page handlers are now invoked `$handler($request, Csp::nonce())` — an additive 2nd arg (a 1-arg handler still works; PHP ignores the extra argument), reflected in the `AdminPageRegistrar` docblock. Regression tests: `HeadContributionTest::test_a_contributor_can_nonce_a_script_matching_the_page_csp`, `PluginAdminPageTest::test_a_handler_receives_the_csp_nonce_as_its_second_argument` + `test_a_one_argument_handler_still_works`. Cross-repo: `nimbus-plugin-seo` test constructions of `PageContext` bumped for the new required param. Note (FU-7): a nonce'd external analytics `<script src>` now *loads*, but its beacon egress is still blocked by the site's `default-src`/absent `connect-src` — self-hosted/proxied analytics works fully; hosted beacons are a separate follow-up.
- **Priority:** P2
- **Type:** product-gap
