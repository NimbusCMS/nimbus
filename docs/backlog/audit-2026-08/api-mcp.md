...
### API-4 · Optimistic concurrency is check-then-act, not an atomic CAS (lost update) ✅ RESOLVED
- **Resolved:** Slice L (branch `slice-l-concurrency-migrations`). The write is now an atomic compare-and-swap: `EntryOperations::update`/`delete` pass the just-read `version` to `EntryService::save`/`delete` (new optional `?int $expectedVersion`), which threads it to `EntryRepository::update`/`delete` where the statement gains `AND version = :expected`; `rowCount() === 0` with a non-null expected throws `EntryConcurrencyConflict`, caught in `EntryOperations` → `preconditionFailed` (412 / MCP `precondition_failed`). Closes the read→write lost-update race for **both PATCH and DELETE** (both lenses: the If-Match contract binds both). Single atomic UPDATE — InnoDB's row lock serializes competing writers, no `SELECT FOR UPDATE` needed. The `version = version + 1` SET guarantees a matched row always changes, so `rowCount` reliably reflects a match (pinned by a no-change-still-succeeds test). Admin path passes no expected version → last-write-wins, unchanged. `If-Match: *` becomes a versioned CAS (stricter — a `*` racing a bump now 412s; accepted, a retry succeeds). Severity: Medium (data-integrity). Tests: `EntryConcurrencyTest` (stale save → conflict + rollback + no event; stale delete → row survives + no event; matching version bumps; no-change succeeds; admin last-write-wins; conflict → 412-not-500 via an injected double). Closes API-7's two-writers matrix row.
- **Priority:** P2
