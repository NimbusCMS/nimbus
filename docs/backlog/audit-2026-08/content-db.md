...
- **✅ RESOLVED** (Slice T) — `EntryView::many()` now batch-expands a page in **O(1)** reference queries instead of ~2N: every media id on the page resolves in one `MediaRepository::findMany()` (the shipped-but-unconsumed primitive gets its first consumer), and each relation field in one new `RelationRepository::liveTargetsFor($entryIds, $fieldId, $target)` (grouped by `from_entry_id`, `sort,id` order per group). **The DATA-1 guards are preserved structurally:** the batched relation query carries the identical declared-target JOIN + published/live filter, and the per-field scope gate stays in `EntryView` (a denied field yields `[]` page-wide and its ids are never fetched). **`one()` now delegates to `many([$row])` — one assembly path, so the per-row and batched results cannot drift on the guards** (single-entry callers pay one 1-element `IN` query, same count as before). Resolved on **arithmetic evidence** (1+50+50 at `MAX_PER_PAGE`), not a load incident. Ledger note: this closes the F1 N+1 arm; the F1 "3rd reference-resolving field type → public reference-expansion capability" arm **stays open** (still only media + relation). Tests: `RelationIntegrityTest` batch cases (no cross-row graft; scope gate page-wide) + the existing per-row guard suite, now routed through the batch, stays green.
- **Priority:** P3
- **Type:** performance
