...
### 2026-08-26 · Public "try it" demo (demo.nimbuscms.dev) — reviewed pre-build; High closed
- **Design reviewed** (anonymous public → full `admin`, shared instance, hourly reset). Confidentiality containment confirmed solid (separate schema + own-schema-only user, cross-schema DENIED, pinned trusted-proxy, uploads no-PHP, plugins not visitor-toggleable, dangling `site.home` degrades not-500, reset erases ALL app state incl. minted tokens). **One HIGH:** "contained to the demo" broke at the shared layer — one MySQL + one 35 GB disk for all sites, so a public admin could mint tokens → bulk API/upload → exhaust the shared disk/connections and degrade the MARKETING site; the hourly reset only truncates.
- **Closed the High + must-ships:** (1) **demo mode disables API-token minting** server-side on BOTH paths — `TokensController::store` (`?err=demo-disabled`) and MCP `TokensToolset::mintToken` (`ToolResult::error 'forbidden'`) — removes the bulk-abuse amplifier; tested (admin + MCP, zero rows). (2) Demo gets its **own MySQL container + volume** (isolates connection pool/locks/buffer + DB data) and a **size-watchdog reset** hard-capping its disk footprint. (3) **Atomic reset** from a pre-baked root-owned golden dump the demo container can't reach, scoped to only the demo DB, demo stopped during restore; wipes the WHOLE storage volume (not just page cache). (4) Demo `TRUSTED_PROXIES` = edge-Caddy IP ∪ CF ranges (DEP-1) — precondition for the throttle. (5) Cloudflare rate-limit rule on `demo.nimbuscms.dev` (/admin/login,/api/) + Bot Fight Mode. (6) **PDF dropped** from the demo upload allow-list (`MediaUploader`, `Config::demo()`) — malware-carrier via our domain; tested. (7) inherits every standard site control (cloudflare_only + origin cert, own least-priv DB user, DEP-2, non-root/cap_drop/no-socket/mem_limit).
- **Policy call taken:** neuter token-mint (amplifier); KEEP user management (no amplifier, good showcase, reset-sufficient, and change-password already server-refused). Accepted Low residuals with the reset as compensating control: shared-instance self-defacement (self-heals hourly).
- **Regression tests (all green, 1000 suite):** `DemoModeTest` — admin + MCP token-mint refused with zero `nb_api_tokens` rows; `MediaUploaderTest` — PDF refused in demo (proven via the demo message, not the generic type check).
- **Verdict: safe to launch with the 7 controls** (High demoted by #1+#2). Deploy-side controls (#2/#3/#4/#5) verified on the box at go-live (vandalize→reset-heals, cross-site isolation, disk cap). Core-side (#1/#6) locked by tests here.
