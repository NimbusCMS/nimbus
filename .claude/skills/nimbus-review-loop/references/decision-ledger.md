...
### 2026-08-25 · Public `$nav` view-model — collection navigation for themes [Core]
- **Classification: Core** (SiteController view-model + theme-contract addition), secondary Theme (the consuming sidebar). Both Fable skills reviewed the design before build → ship (revised). Found by dogfooding nimbuscms.dev's docs surface: entry templates can't render a collection sidebar (the index already carries fields via `EntryView::many`; only entry pages lacked siblings).
- **Change:** `SiteController::nav(Collection)` provides `$nav` to both `renderCollection` + `renderEntry` — the collection's **live** entries (same shape as `$entries`) via `view->many($c, liveForCollection($c->id, NAV_MAX=200, 0))`. Memoized per request; **always defined** (`[]` otherwise). **Two gates:** `isPubliclyBrowsable()` (singleton/blocks → `[]`, no links to SVM-4 404s) AND an **opt-in `nav` key in theme.json** (list of handles or `true`) — theme.json's first *runtime* read, hardened (absent/malformed → no nav, never 500).
- **Rulings from the burst:** FULL `many()` not a lean projection (many() is already O(1)-batched per DATA-5; lean wouldn't avoid decoding the data blob where section/order live, and would leak raw storage + fork the entry shape — a lean `EntryView::summaries()` is named-and-deferred). Opt-in via the manifest, NOT always-provide (always would tax every high-traffic entry page of themes that never use nav) and NOT a handle-taking closure (that becomes a site-wide enumeration primitive; data-only wins). NAV_MAX=200 (2× MAX_BLOCKS), truncates in index order (documented like SITEMAP_MAX). Cache-clean: no query input → no new cache-key surface (HTTP-6 intact); staleness handled by the existing full-flush-on-write.
- **DoD met:** implemented + integrated (one memoized builder, both paths) + verified (`SiteNavTest` ×6: draft/scheduled excluded on index+entry; toApi-not-raw via a transforming fixture type; singleton→[]; not-opted-in→[]; no-manifest→[]; single-collection isolation — plus `CacheRoutesTest` stays green) + documented (COMPATIBILITY theme-contract §, starter manifest note). Mobile: the sidebar's 375px collapse is verified on the CONSUMER (nimbuscms-www docs templates, next). MCP: N/A — read-parity pre-exists via the read API/MCP tools.
- **Recorded follow-ups (pre-existing, non-blocking):** lean `summaries()` seam (perf, when a heavy opted-in collection hurts); `blocks()` uses per-row `view->one` where `many` would batch (Low, MAX_BLOCKS-bounded); `homePage()` doesn't consult `isPubliclyBrowsable` for an odd `blocks`/home setting (pre-existing). theme.json is now a runtime contract surface (evolving pre-1.0).

### 2026-08-25 · Agent discoverability (AEO) — SPLIT: generator meta (plugin) + /llms.txt (core) [Core + Official plugin]
- **Context:** Dan asked to make the site "agent discoverable" as a feature of `nimbus-plugin-seo`. Both Fable skills reviewed the design before build; the platform loop **split the feature and rejected the speculative part** — a genuinely load-bearing review outcome.
- **REJECTED (all three proposed head forms):** a `<link rel="mcp-server">` (unregistered rel, no consumer), a bespoke `<script type="application/json">` discovery block (private format, zero consumers), and a schema.org `potentialAction`/`EntryPoint` (semantically dishonest — Actions describe consumer actions, not a bearer-token JSON-RPC management surface). Speculative-metadata guard applied: no agent consumes head-level MCP hints today. Drift Guard: absent nimbuscms.dev we would NOT ship a custom discovery format to every install. **Ledger watch item:** when an MCP discovery convention with real consumers stabilizes (a well-known URI or a registry), add it — in CORE, beside robots.txt.
- **SHIPPED (plugin, PR plugin-seo#2, merged a83b866):** a versionless `<meta name="generator" content="NimbusCMS">` head contributor — a real, widely-consumed convention; constant, no version (fingerprinting), no PageContext value (escaping/injection sink).
- **SHIPPED (core, this slice):** `/llms.txt` (llmstxt.org) served by `SiteController` beside robots.txt — the agent-facing sibling. Core facts advertised by core (ADR 0004 split: root-file foundations in core, opinionated per-page metadata in the plugin). Content: H1 site name + description blockquote + a **core-authored** note that the site is operable over MCP (endpoint from `Config::appUrl()`, token-gated, guide via `nimbus://guide/core`) + publicly browsable collections (SVM-4 gate — no singletons/blocks) + a sitemap pointer. NO version strings (CMS or MCP-protocol — the latter is negotiated at initialize). Untrusted values (title/description/collection names) FLATTENED to a single line so a newline can't forge a `##` section an agent would read as instruction.
- **DoD met:** implemented + integrated (real route, text/plain) + verified (`SiteRoutesTest`: advertises endpoint+guide+sitemap, browsable-only, no CMS version, flatten-resists-section-forgery) + documented (COMPATIBILITY, plugin README). Security-reviewed on the built route (new public endpoint). Mobile: N/A (text route). MCP: this IS the agent-surface advertisement.
- **Lesson:** the review's speculative-metadata guard stopped a private discovery format from freezing into every install's markup — "advertise a convention agents actually consume, in the layer that owns the fact."
