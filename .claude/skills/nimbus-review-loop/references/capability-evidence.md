...
| **Event subscription** (`PluginContext::events()`, `CoreEvents::REQUEST_HANDLED`) | `nimbuscms/analytics` (records page views) | **1** (Analytics) | Public ([COMPATIBILITY](../../../../docs/COMPATIBILITY.md)) | pre-1.0 | `request.handled` is best-effort/isolated; entry-event payloads not frozen | a 2nd unrelated listener (webhooks / activity log) |
...
| **Storage** (`PluginContext::storage()`, ADR 0005) | `nimbuscms/analytics` | **1** (Analytics) | Public | pre-1.0 | own tables only — a **contract, not a sandbox**; no core-data access (that is a later tiered contract) | a 2nd storage-owning plugin; Tier 1 (read core content) when needed |
