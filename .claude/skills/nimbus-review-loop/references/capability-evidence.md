...
| **Maintenance** (`PluginContext::maintenance()`, `nimbus prune`) | `nimbuscms/analytics` (`analytics_hits` retention); `nimbuscms/api-advanced` (`api_audit_log` retention) | **2** (Analytics, Api-advanced) | Public | pre-1.0 | tasks are `callable():int`; run only by the CLI (no scheduler in core); own tables only | **broadly proven** — born with two consumers; a scheduler is the next need |
